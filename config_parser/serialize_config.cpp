...
void serialize_config::serialize(){
    ...
    for(int a = 0; a < ipsec_vpn.size(); a++){
        for(int b = 0; b < ipsec_vpn.size(); b++){
            ...
            if(b != a){
                if(data::tokenize_ipv4(ipsec_vpn[a].remote_policy.ipv4)[2] == data::tokenize_ipv4(ipsec_vpn[b].remote_policy.ipv4)[2]){
                    ...
                    printf("VPN IP %s in %s has the same remote subnet as %s! Changing %s to %s as placeholder\n", ipsec_vpn[a].remote_policy.ipv4.c_str(), ipsec_vpn[a].name.c_str(), ipsec_vpn[b].name.c_str(), ipsec_vpn[a].name.c_str(), _new_ip.c_str());
                    ipsec_vpn[a].remote_policy.ipv4 = _new_ip;
                }
                if(data::tokenize_ipv4(ipsec_vpn[a].local_policy.ipv4)[2] == data::tokenize_ipv4(ipsec_vpn[b].local_policy.ipv4)[2]){
                    std::vector<std::string> _tmp = data::tokenize_ipv4(ipsec_vpn[a].local_policy.ipv4);
                    std::string _new_ip = _tmp[0] + "." + _tmp[1] + "." + std::to_string(data::random_number(20)) + "." + _tmp[3];
                    printf("VPN IP %s in %s has the same local subnet as %s! Changing %s to %s as placeholder\n", ipsec_vpn[a].local_policy.ipv4.c_str(), ipsec_vpn[a].name.c_str(), ipsec_vpn[b].name.c_str(), ipsec_vpn[a].name.c_str(), _new_ip.c_str());
...
serialize_config::virtual_server serialize_config::parse_virtual_server_object_to_struct(std::string s){
    ...
    for(int a = 0; a < _tok.size(); a++){
        ...
        if(_tok[a] == "virtual-server"){
            buff.name = _tok[a+1];
        }
        else if(_tok[a] == "interface"){
            buff.inte = _tok[a+1];
        }
        else if(_tok[a] == "source-ip"){
            buff.source_ipv4 = _tok[a+1];
        }
        else if(_tok[a] == "original-ip"){
            buff.original_ipv4 = data::search_address_object(_tok[a+1]).ipv4;
        }
        else if(_tok[a] == "map-to"){
            buff.map_to_ipv4 = data::search_address_object(_tok[a+1]);
        }
        else if(_tok[a] == "map-type"){
            buff.map_type = _tok[a+1];
            if(_tok[a+1] == "original-service"){
                buff.external_service = data::search_service_object(_tok[a+2]);
            }else if(_tok[a+1] == "service-group"){
                parse_virtual_server_service_group(buff, data::search_object_group(_tok[a+2]), ranges);
            }
        }else if(_tok[a] == "mapped-service"){
            buff.internal_service = data::search_service_object(_tok[a+1]);
        }
    }

    /*
    Nebula does not like range ports and single ports in the same entry so we need to 
    save the range ports into a seperate buffer and create a new entry with the sama ipv4 and interface etc.
    but only the ranges
    */
    if(ranges.size() > 0){
...
void serialize_config::serialize_interface(std::string iface){
    ...
    */
    for(data::tokenized_stuff s : p.find_block("interface " + iface, "ip address")){
        ...
        _buff.name = data::tokenize(s.name)[1];
        /*
...
serialize_config::ip_address_config_style_interface serialize_config::parse_interface_ip_address(std::string s){
    ...
    if(buffer[2] == "dhcp"){
        buff.type = "DHCP";
        if(buffer[3] == "metric")
            buff.metric = buffer[4];
        buff.ipv4 = "192.168.1.1";
        buff.subnet = "255.255.255.0";
    }else {
        buff.ipv4 = buffer[2];
        buff.subnet = buffer[3];
        buff.type = "STATIC";
    }

    //printf("\n%s, %s\n\n", buff.ipv4.c_str(), buff.subnet.c_str());
    return buff;
}
...
serialize_config::ip_dhcp_pool_interface serialize_config::parse_dhcp_pool(std::string s){
    ...
    /*
    If the interface arent a dhcp-server it doesnt need a ip address nor pool size
    but nebula does not accept those to be null se here we set it to some default
    ...
    */
    ...
    for(std::string raw : dhcp_pool[0].raw){
        if(raw.find("network") != std::string::npos){
            ...
            if(raw.size() >= 5){
                ...
                printf("Lease for %s, could not be parsed! using default 2,0,0\n", s.c_str());
                buff.lease = "200";
            }
        }
    }
...
serialize_config::crypto_vpn serialize_config::parse_crypto_map(data::tokenized_stuff map){
    ...
    for(std::string raw : map.raw){
        if(raw == "activate" || raw == "deactivate"){
            ...
            _tmp.adjust_mss = data::tokenize(raw)[1];
        }else if(raw.find("ipsec-isakmp") != std::string::npos){
            _tmp.ipsec_isakmp = parse_isakmap(p.find_single_block("isakmp policy " + data::tokenize(raw)[1]));
        }
        else if(raw.find("scenario") != std::string::npos){
            _tmp.scenario = data::tokenize(raw)[1];
        }
        else if(raw.find("encapsulation") != std::string::npos){
            _tmp.encapsulation = data::tokenize(raw)[1];
        }
        else if(raw.find("transform-set") != std::string::npos){
            _tmp.transform_set = data::tokenize(raw)[1];
        }
        else if(raw.find("security-association") != std::string::npos){
            _tmp.secure_associtaion = data::tokenize(raw)[4];
        }
        else if(raw.find("pfs") != std::string::npos){
            _tmp.pfs = data::tokenize(raw)[2];
        }  
        else if(raw.find("local-policy") != std::string::npos){
            _tmp.local_policy = data::search_address_object(data::tokenize(raw)[1]);
            ...
            _tmp.remote_policy = data::search_address_object(data::tokenize(raw)[1]);
        }
        else if(raw.find("conn-check") != std::string::npos){
            _tmp.conn_check = data::tokenize(raw)[2];
        }
    }
